...
$("automationPanel").innerHTML='<article class="card"><h3>🔌 مسار البيانات</h3><p>'+esc(h.pipeline_note)+'</p><div class="tags"><span class="tag">signals '+num(h.signals)+'</span><span class="tag">opportunities '+num(h.opportunities)+'</span><span class="tag">engine '+num(h.engine_runs)+'</span><span class="tag">approvals '+num(h.pending_approvals)+'</span><span class="tag">osint '+num(h.osint_events)+'</span></div></article><article class="card"><h3>🛡️ حدود التنفيذ</h3><p>التواصل منخفض المخاطر يمكن أن يؤتمت؛ التفاوض، الالتزامات، الدفع، التحويل والتداول الحساس تبقى تحت Policy + Approval.</p><div class="actions-inline"><button data-action="dashboard_refresh" class="primary">إعادة فحص</button><button class="danger" onclick="toast(&quot;لا يوجد تنفيذ مالي تلقائي&quot;)">سلامة التنفيذ</button></div></article>';
}
renderControlPlane();
